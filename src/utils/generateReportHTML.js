// src/utils/generateReportHTML.js
export function generateWeeklyReportHTML(email, habits) {
  const COLORS = {
    bgMain: '#0f172a',
    bgCard: '#1e293b',
    textPrimary: '#ffffff',
  ...
  const countDays = (habit) => Object.values(habit.history || {}).filter(Boolean).length

  const calculateStreak = (habit) => {
    const dates = Object.entries(habit.history || {}).sort((a, b) => b[0].localeCompare(a[0]))
    let streak = 0
